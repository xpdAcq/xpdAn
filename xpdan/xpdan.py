class xpdAn:
    ...
    def _all_headers(self):
        ...
        search_dict = _default_dict.update(dict(bt_saf_num=self.saf_num))
    ...
    def list(self, *, group_index=None, **kwargs):
        ...
        if kwargs is None:
            ...
            if len(kwargs) != 1:
                print("WARNING: we only allows single criteria now")
        ...
        if kwargs is None:
            ...
            for ind, el in enumerate(group_list):
                print("Required field: {} resulting in following grouping:\n"
                      "has {} headers under current"
                      "SAF number".format(filed, len(h_list)))
            #FIXME
