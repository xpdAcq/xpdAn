    ...
    def event(self, doc):
        ...
        fields = [
            ...
            if (
                ...
                and "external"
                in self.descriptor_dict[doc["descriptor"]]["data_keys"][field]
            )
        ]
        if fields:
            if self.db is None:
...
class LiveTiffExporter(BrokerCallbackBase):
    ...
    def __init__(
        self, field, template, dryrun=False, overwrite=False, db=None
    ):
        try:
            import tifffile
        except ImportError:
    ...
    def _save_image(self, image, filename):
        if not self.overwrite:
            if os.path.isfile(filename):
                raise OSError(
                    "There is already a file at {}. Delete "
        ...
        if image.ndim == 3:
            for i, plane in enumerate(image):
                filename = self.template.format(
                    i=i, start=self._start, event=doc
                )
