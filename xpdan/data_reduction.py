...
#
##############################################################################
from xpdan.data_reduction_core import *
...
int_save_kwargs = {}
for k, v in inspect.signature(integrate_and_save).parameters.items():
    if k in an_glbl.keys():
        int_save_kwargs[k] = an_glbl[k]

int_save_kwargs.update({'db': an_glbl['exp_db'], 'save_dir': an_glbl['tiff_base']})

tiff_save_kwargs = {}
for k, v in inspect.signature(save_tiff).parameters.items():
    if k in an_glbl.keys():
        tiff_save_kwargs[k] = an_glbl[k]

tiff_save_kwargs.update({'db': an_glbl['exp_db'], 'save_dir': an_glbl['tiff_base']})
