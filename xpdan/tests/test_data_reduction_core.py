...
def test_integrate_core_smoke(
    exp_db, fast_tmp_dir, kwargs, known_fail_bool, close_mpl_figs
):
    print(kwargs)
    if known_fail_bool:
    ...
    old_times = [
        os.path.getmtime(os.path.join(fast_tmp_dir, f))
        for f in os.listdir(fast_tmp_dir)
    ]
...
@pytest.mark.parametrize(("kwargs", "known_fail_bool"), integrate_kwargs)
def test_integrate_last_core_smoke(
    exp_db, fast_tmp_dir, kwargs, known_fail_bool
):
    print(kwargs)
    if known_fail_bool:
        pytest.xfail("Bad params")
