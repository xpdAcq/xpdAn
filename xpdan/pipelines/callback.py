    ...
    def start(self, doc):
        ...
        if not is_dark:
            if self.write_to_disk:
                yml_name = render_clean_makedir(
                    self.light_template, ext=".yml", raw_start=doc
                )
    ...
    def event(self, doc):
        if self.dark_img is not None:
            ...
            if self.dark_img is not None:
                img -= self.dark_img
            if self.vis:
                self.vis_callbacks["dark_sub_iq"](
                    "event", format_event(img=img)
                )
            if self.write_to_disk:
            ...
            if self.analysis_setting == "full":
                ...
                # get calibration
                if self.is_calibration:
                    calibration, geo = img_calibration(
                        img, self.wavelength, self.calibrant, self.detector
                    )
                    _save_calib_param(
                        calibration,
                        h_timestamp,
                        os.path.join(
                            self.calibration_md_folder, "xpdAcq_calib_info.yml"
                        ),
                    )
                    if self.write_to_disk:
                ...
                if geo:
                    img = polarization_correction(
                        img, geo, self.polarization_factor
                    )

                    # Masking
                    if doc["seq_num"] == 1:
                        if (
                            self.start_doc["sample_name"] == "Setup"
                    ...
                    if self.vis:
                        self.vis_callbacks["iq"](
                            "event", format_event(q=q, iq=iq)
                        )
                    ...
                    if self.vis:
                        self.vis_callbacks["itth"](
                            "event", format_event(tth=tth, iq=iq)
                        )
                    if self.write_to_disk:
                    ...
                    if self.composition:
                        ...
                        if self.vis:
                            self.vis_callbacks["pdf"](
                                "event", format_event(r=r, pdf=gr)
                            )
                        if self.write_to_disk:
